...
*Gemini dropped 2026-09-20 (Victor):* Google closed personal Code Assist sign-in for Gemini CLI ("migrate to Antigravity"), and the consumer Gemini app has no usage endpoint. Antigravity's status-line JSON feed (per-bucket `remaining_fraction` / `reset_time`, pushed while the CLI runs) was the remaining route; Victor chose not to pursue it. The probe-only skeleton was removed with the rest of the batch; the capture notes are in `BUILD_LOG.md` ▸ 2026-09-20. P2-2 is closed with Cursor and Copilot.
*Gemini done 2026-09-21 (via Antigravity):* reopened and shipped by the local route codenotch pointed at — Gemini is Walder's fifth service, read from the running Antigravity IDE's own language server over `127.0.0.1` (`src/providers/antigravity.ts` finds the `language_server_macos_arm` pid, its `--csrf_token` argument and its listening ports, then `POST`s `RetrieveUserQuotaSummary` to whichever port speaks plain HTTP), with `parseAntigravityUsage` turning every `groups[].buckets[]` entry into one window row on `1 − remainingFraction`, never a packet to Google, and `LOGIN.gemini: null` because there is nothing to sign in to.
