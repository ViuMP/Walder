...
**Regenerated strips (2026-09-20):** rows 3, 4, 9 and 10 — golden and dapple `idle_worried` and
`idle_exhausted` — were regenerated by Codex with GPT image generation alongside Strip 21 and
**approved by Victor in the gallery the same day**, replacing the 2026-09-10/11 Firefly versions.
Only those ten frames and the three new `lie` frames changed in the sheet; every other frame is
byte-identical to the previous build. The four final prompts are recorded under §1 and §2 below.
...
## Yuna — approved domestic-cat source archive (2026-09-21)
...
Every set has: `idle` (6), `idle_happy` (5), `idle_worried` (5), `idle_exhausted` (5), `out` (2),
`perk` (6), `tilt` (3), `sleep` (3), `lie` (3), `bark` (4), `walk` (4), `wake` (4), `tail_wag` (3),
`hop` (5), and `pet` (6). The yarn ball is red; `tilt` has no question mark, `sleep` has no sleep
...
### Final GPT regeneration prompts — approved worried and exhausted strips

These are the four final prompts used for the approved replacements, recorded separately because they
supersede the earlier Firefly briefs above. Attach the named references with each prompt.

**Golden worried** — attach approved `v4/golden/idle.png`.

> Create one complete 1376×768 pixel-art sprite strip: exactly five identical golden long-haired miniature
> dachshunds in a single horizontal row on a uniform light-grey background, all facing left in three-quarter
> view, both eyes visible, evenly spaced, clear of every canvas edge, and with all paws on one ground line.
> Match the attached approved idle strip's proportions, large round dark eyes with one white highlight,
> feathered ears, cream chest, plume tail, crisp Pokémon Black/White-style pixels and hard warm-brown outline.
> Give him only a worried expression: gentle inward brows, a closed downturned mouth, low ears and tail, plus
> one small light-blue sweat drop beside his head in the identical position in every frame. Frames 1–3 are the
> same resting worried dog; frame 4 is that exact dog with both eyes half-closed; frame 5 is that exact dog
> with both eyes closed. No gradients, shadows, floor line, text, props, collar, dividers or extra marks.

**Golden exhausted** — attach approved `v4/golden/idle.png` and the approved golden worried strip.

> Create one complete 1376×768 pixel-art sprite strip: exactly five identical golden long-haired miniature
> dachshunds in a single horizontal row on a uniform light-grey background, all facing left in three-quarter
> view, both eyes visible, evenly spaced, clear of every canvas edge, and with all paws on one ground line.
> Match the attached approved idle and worried strips exactly: same dog, proportions, pixel density, outline,
> ears, tail, feet, chest and the same large round dark eyes with one white highlight. Make him tired only by
> lowering the upper eyelids, flattening the ears back, lowering the head a little and adding one small pink
> tongue; do not invent narrow, almond-shaped or differently drawn eyes. Frames 1–3 are identical resting
> exhausted dogs; frame 4 is that exact dog with eyes almost closed; frame 5 is that exact dog with eyes
> closed. The tongue stays identical in all five. No gradients, shadows, floor line, text, props, collar,
> dividers or extra marks.
...
These reproduce the approved golden regeneration strips frame for frame; attach the matching golden
strip and the dapple puppy photo with each prompt.

**Dapple worried** — attach approved `v4/golden/idle_worried.png` and the dapple puppy photo.

> Create one complete 1376×768 pixel-art sprite strip: exactly five silver-dapple long-haired miniature
> dachshunds in one horizontal row on a uniform green #3FA34D background. Reproduce the attached golden
> worried strip frame for frame: same five poses, size, spacing, ground line, left-facing three-quarter view,
> round eye shape, sweat-drop placement and worried expression. Change only the coat to a consistent silver
> dapple: cool silver-grey base, irregular charcoal and black blotches, tan points on brows, muzzle, chest
> and paws, and ice-blue eyes with one white highlight each. The dapple pattern must be identical on all five
> dogs. No gradients, shadows, floor line, text, props, collar, dividers or extra marks.

**Dapple exhausted** — attach approved `v4/golden/idle_exhausted.png` and the dapple puppy photo.

> Create one complete 1376×768 pixel-art sprite strip: exactly five silver-dapple long-haired miniature
> dachshunds in one horizontal row on a uniform green #3FA34D background. Reproduce the attached golden
> exhausted strip frame for frame: same five poses, size, spacing, ground line, left-facing three-quarter
> view, ears, tongue, eyelid sequence and the same large round eye design used across the other strips—never
> narrow or almond-shaped eyes. Change only the coat to a consistent silver dapple: cool silver-grey base,
> irregular charcoal and black blotches, tan points on brows, muzzle, chest and paws, and ice-blue eyes with
> one white highlight each. The dapple pattern must be identical on all five dogs. No gradients, shadows,
> floor line, text, props, collar, dividers or extra marks.
