...
| 3.4j | ⚠ | **A hidden row is silent, and the face does not care.** Untick **5-hour**, then `Developer ▸ Inject usage ▸ 90%`: **no bubble at all**, and the dog still turns worried — the face follows the Claude 5-hour window whether or not that row is on the card. Now untick something else instead, leave 5-hour ticked, and inject 90 % again: he barks `5-hour: 90% used` as normal | ☐ | ☐ | |
...
| 3.4l | ⚠ | **An exhausted pool is remembered while hidden.** Untick **Codex credits**, let the balance empty (or fake it), then tick the row back on: he does **not** bark `Codex credits: none left` about an emptying that happened while the row was off the card. Only topping the credits up and running them out again gets one bark | ☐ | ☐ | |
