...
| 3.4i | ⚠ | **Show in overview ▸** sits directly under **Card size**, with a ticked checkbox per row (Claude's rows, a separator, ChatGPT's). Hover the dog to bring the card up and, without moving the cursor off him, untick a row: it leaves the card **at once** — not at the next poll — and the card shrinks to fit. Tick it back: it returns in its old position | ☐ | ☐ | |
| 3.4i2 | ⚠ | **Empty a service and it disappears from the card.** Untick **every** Claude row, then hover: the card has **no CLAUDE section at all** — no heading, no "via Claude Code login", no "no limits reported" — and starts straight at CHATGPT. Check at all three card sizes. Tick one Claude row back on: the whole section returns, with that row in it. Then compare against a service that really reported nothing — log out of ChatGPT, or use an account with no Codex windows — which **keeps** its heading and its `no limits reported` line | ☐ | ☐ | |
| 3.4i3 | ⚠ | **The card a fresh panel pulls hides the same rows as a pushed one.** With a row unticked, quit and relaunch Walder and hover **before the first poll lands** (within the first few seconds): the unticked row is **still** off the restored card, and an emptied service is still gone. Until 0.2.6 the pulled copy was unfiltered, so hidden rows came back for up to three minutes after every launch | ☐ | ☐ | |
| 3.4j | ⚠ | **A hidden row is silent, and the face does not care.** Untick **5-hour**, then `Developer ▸ Inject usage ▸ 90%`: **no bubble at all**, and the dog still turns worried — the face follows the Claude 5-hour window whether or not that row is on the card. Now untick something else instead, leave 5-hour ticked, and inject 90 % again: he barks `Claude 5h: 90% used` as normal | ☐ | ☐ | |
| 3.4k | ⚠ | **Re-ticking does not replay what you missed.** With 5-hour ticked, inject 82 % (he barks) and click it away. Untick 5-hour, inject 91 %, then tick it back on and inject 91 % again: **nothing** — the 90 level was crossed while it was hidden and is not re-announced at the moment you un-hide. Inject 96 %: that one does bark, because 95 is a level he has genuinely never said | ☐ | ☐ | |
| 3.4l | ⚠ | **An exhausted pool is remembered while hidden.** Untick **Codex credits**, let the balance empty (or fake it), then tick the row back on: he does **not** bark `Codex credits: 100% used` about an emptying that happened while the row was off the card. Only topping the credits up and running them out again gets one bark | ☐ | ☐ | |
| 3.4m | | The ticks survive a quit and relaunch, and are independent of Size, Card size and Primary service. A row the services have only just invented (one Walder does not name up front) still appears in the list, under the label the card gives it | ☐ | ☐ | |
...
| 5.3 | | Inject **82 %** → worried (ears down, sweat bead) **and** one bark: a bubble reading `Claude 5h: 82% used` | ☐ | ☐ | |
...
| 5.7b | ⚠ | **A higher threshold replaces it in place.** With an 80 % bark up, inject 86 %: the text changes to `Claude 5h: 86% used` on the spot, with no gap in which there is no bubble, and no second bubble queued behind | ☐ | ☐ | |
...
| 5.9a2 | ⚠ | **Two lines still fit.** Force a long bark (`Sonnet weekly: 85% used`) at each of the three sizes and on both a Retina and a non-Retina display: two lines, no ellipsis, nothing clipped by the top of the window | ☐ | ☐ | |
| 5.9b | ⚠ | **Extra usage barks like a window.** With the row present and above 80 %, a bubble reads `Claude credits: 80% used` (the bubble name plus the observed percentage, same as any window) | ☐ | ☐ | |
| 5.9c | ⚠ | **Codex credits bark exactly once, and only when empty.** When the balance runs out he barks `Codex credits: 100% used`; he does **not** bark again on later polls while it stays empty, and does **not** bark at 80/85/90 of anything (a balance has no thresholds). Top the credits up and run them out again → one more bark | ☐ | ☐ | |
| 5.9d | ⚠ | **Capless Extra usage limit reached.** With no monthly limit, let claude.ai report its spend limit reached: one bubble reads `Claude credits: 100% used`. It does not repeat on later refreshes, including after a failed poll; it re-arms only when the provider clears the reached state | ☐ | ☐ | |
...
| 7.7a | | **The fourth entry, `PostToolUse` (0.2.7).** Once a tool call you approved finishes running, a held or shown `?` for that tool clears on its own — no `done`, no bubble of its own, just the retraction | ☐ | ☐ | |
| 7.8 | | Quit Walder and use Claude Code normally: no errors, no delays, no failed hooks | ☐ | ☐ | |
| 7.9 | | **Remove Claude Code hooks…** asks the same way, then takes the four entries out and leaves everything else in the file as it was. Run it again: it says there is nothing to remove | ☐ | ☐ | |
...
| 7.11 | ⚠ | **The wording names the tool.** `Developer ▸ Simulate hook` (or a real session) says exactly `Claude done` and `Claude waiting`, and for Codex `Codex done` and `Codex waiting` — never a bare `woof` or a bare `?`. The pixel `?` still appears by his ear on both waits. A Claude reply and a Codex reply seconds apart both get said; a new Claude prompt clears only the Claude wait | ☐ | ☐ | |
...
| 7.14 | ⚠ | **An untrusted Codex hook is silent, and that is Codex's doing.** Install the Codex hooks, then use Codex **without** trusting them: the menu says installed, and the dog reacts to nothing at all. Run `codex`, type `/hooks`, trust Walder's four entries, and the next finished turn gives `Codex done`. The install dialog must have said this — Walder never writes the trust hash itself | ☐ | ☐ | |
...
| 9.15b | ⚠ | **A manual check that finds nothing still answers.** With the running version the newest there is, **Check for updates now**: the dog says `You're up to date`. Click him and it goes, like any other bubble. The bottom item still reads **Check for updates now** — not "Last check failed" | ☐ | ☐ | |
| 9.15c | ⚠ | **A scheduled check says nothing.** Leave Walder running with **Check for updates automatically** ticked and nothing newer published: no `You're up to date` bubble ever appears on its own, however many six-hourly checks have gone past (the log, with Verbose on, is what shows they ran). Only a check you asked for is answered out loud | ☐ | ☐ | |
| 9.16 | | The dog says `Walder 0.1.3 is out` **once**. Quit and reopen Walder, check again: he does not say it a second time (the version is remembered) | ☐ | ☐ | |
...
| **The 0.2.6 behaviour, watched by the owner on 2026-09-19** | Seen working on the dev build: the three-beat introduction against a reset `introduced` flag (after a fix — the first "Hello" was sent before the page had loaded and was lost; the scene is now replayed on every load), the session registry perking the ears and holding the `?` with no hooks installed, a bark waiting behind a live `?` and arriving on the pet that clears it, Still mode from the tray and from the OS Reduce Motion setting, and the face changing while still. Not seen: the 30-minute stand-down (nobody waited), the liveness check (`process.kill(pid, 0)`, deliberately untested), and VoiceOver reading anything *inside* a window — it announces the windows as web content and stops there; the card also only exists while the pointer is on the dog. Rows 7.15–7.17 and 7a.1–7a.4 |
| **The Claude Code login renewal reaching `renewed`** | Verified live on 2026-09-19 as far as the plumbing goes: with the owner logged into the standalone CLI (`~/.local/bin/claude`) and out of claude.ai, `Developer ▸ Renew Claude Code login now` located the binary, spawned it, saw it exit on its own, fired no hook and announced no session, and logged `claude token renewal: no change` — the correct answer for a token with eight hours left, because Claude Code only refreshes inside its own five-minute window. The `renewed` outcome can only come from the natural run four minutes before expiry, and nobody has watched one yet. Also found on the way: a credential Claude Code has *emptied* (blank tokens, `expiresAt` 0) reads as plain `unavailable`, indistinguishable from never having logged in — a future notice, not a renewal case |
